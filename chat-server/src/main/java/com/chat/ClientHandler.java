            ...
            while (loop) {
                ...
                if (control == '@') {
                    ...
                    switch (command) {
                        ...
                        case ("help"):
                            message.setBody(
                                    "Lista Comandi: -1) @[nome] per utilizzare i messaggi privati -2) messaggio senza header = broadcast -3) /exit per chiudere la sessione -4) /help per visualizzare i comandi -5) /ls per visualizzare gli utenti");
                            break;

                        case ("ls"):
                            String utentiStr = "";
                            for(int i = 0; i < listaClient.size(); i++) {
                                utentiStr.concat(listaClient.get(i).clientUserName + " ");
                            }

                            message.setBody(utentiStr);
                            break;
